...
function EpisodesList({
  ...
  const handleToggleWatched = (id) => {
    ...
    setTempWatched((prev) =>
      prev.includes(id) ? prev.filter((watchedId) => watchedId !== id) : [...prev, id]
    )
  }
  ...
  return (
    <div>
      {episodes.map((ep, idx) => {
        let hidden = false
        if (statusFilter === 'watched' && !tempWatched.includes(ep.id)) hidden = true
        if (statusFilter === 'unwatched' && tempWatched.includes(ep.id)) hidden = true
        ...
        return (
          ...
          >
            ...
            <div>
              Personagens: <span style={ep.characters.length >= 10 ? { color: '#39ff14', fontWeight: 700 } : {}}>{ep.characters.length}</span>
              {ep.characters.length >= 10 && (
                <span title="Episódio popular" style={{ marginLeft: 6, color: '#39ff14', fontSize: 18, verticalAlign: 'middle' }}>⚡</span>
              )}
            </div>
